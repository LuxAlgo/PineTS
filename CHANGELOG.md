...
- **`ta.range` inherited the `na` reset of `ta.highest` / `ta.lowest`**: `ta.range` was computed as `ta.highest - ta.lowest`, so the v0.9.34 change that makes those two reset at `na` also changed `ta.range`, which on TradingView follows different rules. `ta.range` keeps the last `length` _non-na_ values: an `na` bar is not added to the window and the result repeats the previous one (`ta.range(src, 5)` over `1, 2, 7, 4, na, 9` is `8`, where `highest - lowest` gives `0`, and `6` on the `na` bar instead of `na`); it returns `na` until `length` non-na values have been seen; and its maximum starts from the smallest positive double, so a window of negative values measures down from 0 (`ta.range(-5, 1)` is `5`, `ta.range` over `-6, -5, -3` is `6`). `ta.range` now has its own rolling window; `ta.highest` / `ta.lowest` are unchanged. Expected values come from TradingView on hand-built periodic series with `na` holes, including negative values and leading `na` (`tests/namespaces/ta/range-na.test.ts`).
