...
class MockProvider {
  ...
  constructor(dataDirectory) {
    if (dataDirectory) {
      ...
      this.dataDirectory = path__namespace.join(projectRoot, "tests", "compatibility", "_data");
    }
  }
  /**
  ...
  findDataFile(tickerId, timeframe, sDate, eDate) {
    if (!fs__namespace.existsSync(this.dataDirectory)) {
      return null;
    }
    const files = fs__namespace.readdirSync(this.dataDirectory).filter((file) => file.endsWith(".json"));
    if (sDate && eDate) {
