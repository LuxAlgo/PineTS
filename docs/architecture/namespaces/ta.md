...
All TA functions store their state in `context.taState`. A unique `callId` (generated by the transpiler) isolates state for different calls to the same function.
...
### Optimized Stateful TA Contract

The optimized functions from Issue #237 follow a shared execution contract.
The storage layout differs by indicator mathematics, but the lifecycle and
parity rules do not.

1. **State isolation**: the state key includes the call-site and every
   parameter that changes the state shape or result, such as length, bias, or
   multiplier. A dynamic length must never reuse a state created for a
   different length.
2. **Bar lifecycle**: `lastIdx` distinguishes a new bar from an intrabar
   re-execution. A new bar commits the prior tentative state; a same-bar
   execution restores the committed state before applying the updated value.
3. **Evaluation timeline**: recursive indicators and extrema advance state
   only when that call-site is evaluated. Exact-window indicators retain the
   existing source-history backfill used for late and `barstate.islast`
   evaluation; an optimization must not silently change either family's
   established behavior.
4. **Gap and `na` handling**: any state reconstruction must preserve the
   indicator's Pine evaluation semantics. `na` must not corrupt a running
   aggregate or cause a value to appear before the call-site has warmed up.
5. **Numeric parity**: preserve the established summation order whenever a
   mathematically equivalent rolling formula changes observable floating-point
   output.

| Indicator family | Current examples | State representation |
| --- | --- | --- |
| Recursive | EMA, RSI, ATR | committed and tentative scalar state |
| Exact window statistic | SMA, WMA, stdev, BB, CCI, HMA | fixed-size window plus scalar aggregates; optimized paths snapshot only the overwritten slot for intrabar rollback and retain source-history backfill |
| Range/extrema | highest, lowest | call-site execution history, with conditional warm-up preserved |

This contract is intentionally not a single runtime helper. The hot paths have
different scalar fields and numerical requirements, so forcing them through a
generic abstraction would add overhead and obscure each indicator's arithmetic.
Future TA optimizations should select the matching family above, retain this
lifecycle contract, and add dynamic-length, conditional/sparse, `na`
continuity, intrabar rollback, and benchmark coverage before adoption.
